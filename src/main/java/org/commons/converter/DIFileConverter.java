...
public class DIFileConverter {
	...
	public static void excelToCSV(String fileToConvert, int sheetIndex,
			String savePath) {
		convert(fileToConvert, null, sheetIndex, savePath);

	}

	public static void excelToCSV(String fileToConvert, String sheetName,
			String savePath) {
		convert(fileToConvert, sheetName, -1, savePath);

	}

	private static void convert(String fileToConvert, String sheetName,
			int sheetIndex, String savePath) {
		DIExcelManager excelManager = new DIExcelManager();
		...
		for (int i = 0; i <= wb.getSheetAt(0).getLastRowNum(); i++) {
			if (sheetName == null || sheetName.isEmpty()) {
				...
				cellList = excelManager.getAllCellList(wb, sheetIndex, i); // First
																			// sheet
																			// as
																			// default
			} else {
